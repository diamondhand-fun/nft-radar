...
export async function inspectLaunch(input, client = radarClient(), selectedToken) {
  ...
  if (!isAddress(value) && !isHash(value)) throw new RadarError("Use a token address or launch transaction hash.");
  if (selectedToken !== undefined && (typeof selectedToken !== "string" || !isAddress(selectedToken))) throw new RadarError("Invalid selected token.");
  if (await client.getChainId() !== chain.id) throw new RadarError("The RPC returned a different chain. Verification stopped.");
  let token, hash;
  if (isAddress(value)) {
    token = value;
    if (selectedToken && selectedToken.toLowerCase() !== token.toLowerCase()) throw new RadarError("The selected token does not match the input.");
    ...
    if (!isHash(hash)) throw new RadarError("No launch found in the recent history range. Use its transaction hash.");
  } else hash = value;
  ...
  if (!isHash(receipt.transactionHash) || receipt.transactionHash.toLowerCase() !== hash.toLowerCase() || !isHash(receipt.blockHash) || typeof receipt.blockNumber !== "bigint" || receipt.blockNumber < 0n)
    throw new RadarError("The RPC returned an inconsistent transaction receipt.");
  if (receipt.status !== "success") throw new RadarError("This transaction has not confirmed a successful launch.");
  ...
  if (expected && !tokens.has(expected.toLowerCase())) throw new RadarError("No matching token launch was found in this transaction.");
  if (!expected && tokens.size !== 1) throw new RadarError(tokens.size ? "This transaction contains several launches. Select a token." : "No supported factory launch found in this transaction.");
  ...
  if (!isHash(snapshot.hash) || typeof snapshot.number !== "bigint" || snapshot.number < receipt.blockNumber)
    throw new RadarError("The RPC returned an invalid metadata block.");
  ...
  if (!isHash(block.hash) || block.number !== receipt.blockNumber || block.hash.toLowerCase() !== receipt.blockHash.toLowerCase())
    throw new RadarError("The launch block changed. Retry once the chain settles.");
  if (!isHash(checkedSnapshot.hash) || checkedSnapshot.number !== snapshot.number || checkedSnapshot.hash.toLowerCase() !== snapshot.hash.toLowerCase())
    throw new RadarError("The metadata block changed. Retry once the chain settles.");
  ...
  if (!source || typeof name !== "string" || !name.trim() || name.length > 256 || typeof symbol !== "string" || !/^[A-Za-z0-9]{1,16}$/.test(symbol) || !imageUrl)
    throw new RadarError("This token does not contain supported Zecbit metadata.");
  return {
